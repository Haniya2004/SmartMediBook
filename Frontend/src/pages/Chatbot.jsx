...
function Chatbot() {
  ...
  // Extract patient_id only for patients (doctors don't have patient_id)
  const patientId = useMemo(() => {
    if (!storedUser || storedUser.role !== "patient") return null;
    return storedUser.patientId || storedUser.patient_id || null;
  }, [storedUser]);
  ...
  }, [storedUser, patientId, token, sessionId]);
  ...
  const getWelcome = () => {
    ...
    if (storedUser.role === "doctor") {
      return "👋 Hello Doctor! I can help you view appointments, see the next patient, check urgent queue, complete appointments, and cancel appointments with a reason.";
    }
  ...
  const sendMessage = async () => {
    ...
    // Prepare the request payload based on role
    const payload = {
      message: trimmed,
    ...
    // Only add patient_id if user is a patient (doctors don't have patient_id)
    if (storedUser?.role === "patient" && patientId) {
      payload.patient_id = patientId;
    }

    console.log("📤 Sending to AI:", {
      ...payload,
      token: payload.token ? `${payload.token.substring(0, 30)}...` : null
    });
    ...
    try {
      ...
      const reply = response?.data?.reply || "Sorry, I could not process that right now.";
      ...
      if (shouldRefreshAppointments(reply)) {
        console.log("🔄 Appointment changed, refreshing dashboard");
        ...
        <div className="w-full h-screen max-w-2xl flex flex-col overflow-hidden shadow-2xl rounded-xl bg-gradient-to-b from-[#3b1d5c] to-[#0f172a] border border-purple-700/40 my-20">
          ...
          <div className="text-center text-sm text-purple-200 py-2 bg-purple-500/30">
            ...
            {storedUser?.role === "doctor" && (
              <span className="ml-2 text-yellow-300">(Doctor Mode)</span>
            )}
            {storedUser?.role === "patient" && (
              <span className="ml-2 text-blue-300">(Patient Mode)</span>
            )}
          </div>
          ...
          <div className="p-4 bg-[#0f172a] border-t border-purple-700/40">
            <div className="flex items-center gap-3">
              <input
                type="text"
                placeholder={storedUser?.role === "doctor" ? "Ask me about appointments, cancellations..." : "Ask me about booking, checking, or cancelling appointments..."}
